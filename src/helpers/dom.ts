...
const dom = {
  ...
  insertCharacter: ({ character, textarea }: { character: string, textarea: HTMLTextAreaElement }) => {
  ...
  copyToClipboard: (str: string, link: string) => {
    if (process.env.VUE_APP_SSR) return
  ...
  loadScript: ({ url, attributes }: { url: string, attributes?: { key: string, value: string }[] }) => new Promise<void>(resolve => {
    if (process.env.VUE_APP_SSR) return
  ...
  loadLink: ({ url, attributes }: { url: string, attributes: { key: string, value: string }[] }) => new Promise<void>(resolve => {
    if (store.getters.lazyLoadedScriptUrls.includes(url)) {
      resolve()
      return
    }
