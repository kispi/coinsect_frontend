...
export interface VuexStateMarketInfo {
  indices: any
  crypto: any
  nasdaq: any
  kospi: any
  assetsIncludingMetal: any
  markets: Record<string, any>
  walletStatus: Record<string, any>
  orderbooks: Record<string, any>
  instruments: Record<string, any>
  leaderboard: any
  realTimeTickers: Record<string, any>
  rawWebsocketInfo: Record<string, any>
  symbols: Record<string, string>
}

const marketInfo: Module<VuexStateMarketInfo, RootState> = {
  state: () => ({
    indices: null,
    crypto: null,
    nasdaq: null,
  ...
  actions: {
    ...
    async loadWalletStatus({ commit, getters }) {
      ...
      const o = {} as Record<string, { d: boolean; w?: boolean }>
      try {
        if (bEx === 'upbit') {
          const data = await $httpNoAuth.get('https://ccx.upbit.com/api/v1/status/wallet') as WalletStatusUpbit[]
          data.forEach(w => o[w.currency] = { d: w.wallet_state === 'working' })
        }

        if (bEx === 'bithumb') {
          const { data } = await $httpNoAuth.get('https://api.bithumb.com/public/assetsstatus/ALL') as {
