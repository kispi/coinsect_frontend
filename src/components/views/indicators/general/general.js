...
const items = [{
  ...
  description: `
    비트코인은 역사적으로 200주 이동평균선에서 바닥을 찾으며 계속 랠리해왔다.
    히트맵 상단에 가까울수록 시장이 매우 과열된 상태이며(매도 신호), 파란색에 가까울수록 바닥에 근접한 것으로 볼 수있다.(매수 신호)
    만약 장기홀더가 아니고 마켓을 타이밍하고 싶은 스윙 트레이더라면, 과열시 팔고 충분히 몇달간 인내심을 갖고 시장을 지켜보다 200 WMA 근처에서 사는 전략이 유효할 수 있다.
  `,
}, {
  link: 'https://www.lookintobitcoin.com/charts/bitcoin-logarithmic-growth-curve/',
  title: 'Bitcoin Logarithmic Growth Curves',
  description: `
    비트코인은 변동성으로 악명높지만, 시간이 지남에 따라 글로벌하게 채택되고 있고 이 추세는 아마 앞으로도 계속될 것이다.
    이 지표는 과거의 비트코인의 가격 움직임으로 향후의 가격 움직임을 예측하려는 여러 모델 중 하나로, 비트코인의 가격이 일련의 로그함수들의 경계 내에서 움직인다는 것을 포착하여 여러개의 로그함수를 중첩하여 나타낸 모델이다.

    그래프가 우측(시간)으로 진행됨에 따라 위로 올라가는 정도(가격)가 크게 줄어드는 로그함수의 특징처럼, 비트코인에 투자해서 얻는 수익률도 시간이 지남에 따라 감소하는 형태를 띄고 있음을 알 수 있다.(* Benjamin Cowen: diminishing return)

    어떤 모델이든 깨지는 순간이 올때까지는 맞는데, 이 글을 쓰는 현재(2022-05-21)의 가격은 지지역할을 하는 하단 로그함수에 위치해 있다.

    현재 기준으로 로그함수의 최상단은 약 $200,000인데, 근시일내에 도달할 가능성은 사실상 희박해보여 로그함수의 곡률이 너무 초창기였던 11년도 랠리를 제외하고 다시 계산되어야 하는게 아닌가 하는 생각도 든다.
  `,
}, {
  ...
  description: `
    ...
    해당 자원이 새로 생성되는 양과, 이미 존재하는 양의 비로 계산되며, 이 비가 높을수록 희소하다고 볼 수 있다.
  ...
  description: `
    ...
    최근 2년간 대단히 빠른 속도로 '바닥'이 올라온 것을 볼 수 있다.
  `,
}]
