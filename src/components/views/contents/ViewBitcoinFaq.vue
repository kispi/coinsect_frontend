<template>
  <div class="view-bitcoin-faq">
    <div class="section">
      <div class="title m-b-24">비트코인 이해하기</div>
    </div>
    ...
    <div class="section">
      <div class="f-12 no-select">What is <span @click="showOrangePill = !showOrangePill" class="orange text-underline cursor-pointer">Orange Pill</span> 💊?</div>
      <article
        ...
        <ul>
          <li>비트코인 커뮤니티에서는, 미국 달러를 위시해 1971년대 이래로 이어져온 신용화폐가 사실 거대한 사기극에 가까운 실패임을 깨닫고, 비트코인이야말로 여태 인류가 발견한 가장 우월한 형태의 돈임을 이해하는 과정을 <span class="orange">Orange Pill 💊</span>을 복용하는 것에 빗대어 설명한다.</li>
...
export default {
  setup() {
    ...
    const callApi = async () => {
      try {
        await store.dispatch('loadBitcoinArticles')
        items.value = store.getters.bitcoinArticles.map(o => ({ ...o, $$expanded: false }))
      } catch (e) {
